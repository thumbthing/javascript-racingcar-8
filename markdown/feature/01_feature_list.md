...
1. 배열로 변한된 배열의 길이를 확인한다
    - 배열의 길이가 2보다 작은지 확인한다
2. 배열로 변환된 배열을 순회한다
    - `some()`으로 순회한다
        - 배열내의 요소의 길이가 6 이상 또는 0 인지 확인한다
        - 해당 기준에 하나라도 통과한다면 `true`를 반환한다
3. 배열내 요소에 중복된 요소가 존재하는지 확인한다
    - `new Set().size`로 중복 요소를 제거한다
    - 중복 요소가 제거된 `Set`과 기존 배열의 길이가 다를 경우 `true`를 반환한다
